    ...
    <form method="POST" action="{{ route('jadwal.update', $jadwal->id) }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="date" name="tanggal" id="tanggal"
                ...
                value="{{ old('tanggal', optional($jadwal->tanggal)->format('Y-m-d')) }}" required>
            @error('tanggal')
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="time" name="jam_buka_pagi" id="jam_buka_pagi"
                    ...
                    value="{{ old('jam_buka_pagi', optional($jadwal->jam_buka_pagi)->format('H:i')) }}" required>
                @error('jam_buka_pagi')
                <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="time" name="jam_tutup_pagi" id="jam_tutup_pagi"
                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#1e3a8a] @error('jam_tutup_pagi') border-red-500 @enderror"
                    value="{{ old('jam_tutup_pagi', optional($jadwal->jam_tutup_pagi)->format('H:i')) }}" required>
        ...
        <div>
            ...
            <select name="status_pagi" id="status_pagi"
                class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#1e3a8a] @error('status_pagi') border-red-500 @enderror"
                required>
                <option value="buka" {{ old('status_pagi', $jadwal->status_pagi) == 'buka' ? 'selected' : '' }}>Buka</option>
                <option value="tutup" {{ old('status_pagi', $jadwal->status_pagi) == 'tutup' ? 'selected' : '' }}>Tutup</option>
            </select>
            @error('status_pagi')
            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="time" name="jam_buka_sore" id="jam_buka_sore"
                    ...
                    value="{{ old('jam_buka_sore', optional($jadwal->jam_buka_sore)->format('H:i')) }}" required>
                @error('jam_buka_sore')
                <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="time" name="jam_tutup_sore" id="jam_tutup_sore"
                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#1e3a8a] @error('jam_tutup_sore') border-red-500 @enderror"
                    value="{{ old('jam_tutup_sore', optional($jadwal->jam_tutup_sore)->format('H:i')) }}" required>
        ...
        <div>
            ...
            <select name="status_sore" id="status_sore"
                class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#1e3a8a] @error('status_sore') border-red-500 @enderror"
                required>
                <option value="buka" {{ old('status_sore', $jadwal->status_sore) == 'buka' ? 'selected' : '' }}>Buka</option>
                <option value="tutup" {{ old('status_sore', $jadwal->status_sore) == 'tutup' ? 'selected' : '' }}>Tutup</option>
            </select>
            @error('status_sore')
            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
            @enderror
